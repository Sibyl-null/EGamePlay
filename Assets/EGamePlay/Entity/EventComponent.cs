...
namespace EGamePlay
{
    ...
    public sealed class EventComponent : Component
    {
        ...
        private readonly Dictionary<Type, List<object>> _typeEvent2ActionLists = new();
        private readonly Dictionary<string, List<object>> _fireEvent2ActionLists = new();
        ...
        {
            if (_fireEvent2ActionLists.TryGetValue(eventType, out var actionList))
            {
                var tempList = actionList.ToArray();
                foreach (Action<T> action in tempList)
                {
        ...
        public void OnEvent<T>(string eventType, Action<T> action) where T : Entity
        {
            ...
            {
                actionList = new List<object>();
                _fireEvent2ActionLists.Add(eventType, actionList);
            }
            actionList.Add(action);
        }
